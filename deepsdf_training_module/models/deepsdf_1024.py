...
class DeepSDFDecoder1024(nn.Module):
...
class DeepSDFDecoder1024v2(nn.Module):
    ...
    def __init__(self, latent_size):
        ...
        # TODO: Define model
        self.lin1 = nn.Sequential(
            nn.utils.weight_norm(nn.Linear(latent_size + 3, 2048)),
            nn.ReLU(),
            nn.Dropout(dropout_prob),

            nn.utils.weight_norm(nn.Linear(2048, 2048)),
            nn.ReLU(),
            nn.Dropout(dropout_prob),
            ...
            nn.utils.weight_norm(nn.Linear(2048, 2048-(latent_size + 3))),
            nn.ReLU(),
            nn.Dropout(dropout_prob)
